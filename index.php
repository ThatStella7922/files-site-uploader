    ...
    <body>
        ...
        if(isset($_POST['submit'])){ //Begin with a check to see if the form was submitted!
            ...
            if (empty($_POST["destinationDirectory"])) {
                ...
                $validationDidError = false; // No error, proceed with upload
            }

            if (!is_dir($_POST["destinationDirectory"])){
                if (!empty($friendly_destination_directory)) {
                    $statusMessage = "<p>An error occurred while uploading the file:<br>Destination directory was not found: $friendly_destination_directory</p>";
                } else {
            ...
            if (!$validationDidError) {
                // Only proceed with upload if everything checks out (no Error)

                $destination_filepath = $destination_directory . "/" . basename($_FILES["inputFile"]["name"]);
                $friendly_destination_filepath = $friendly_destination_directory . "/" .  basename($_FILES["inputFile"]["name"]);
                $full_destination_filepath = $full_destination_directory . "/" .  basename($_FILES["inputFile"]["name"]);
                ...
                try {
                    move_uploaded_file($_FILES["inputFile"]["tmp_name"], $full_destination_filepath);
        ...
        <div id="master">
            ...
            <div id="form-container">
                <form method="post" action="" enctype="multipart/form-data">
                    ...
                    <p>Destination path: <input type="text" name="destinationDirectory" id="destinationDirectory">/<br>(relative to files.thatstel.la, do NOT include trailing slash)<span class="error"><?php echo $noDestinationDirectoryError;?></span></p>
